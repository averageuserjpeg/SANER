...
# =========================================================
# --- ELECTROMAGNET CLUSTER (RANDOMIZED INSIDE MAZE) ---
# =========================================================
...
for i in range(cols):
    for j in range(rows):
        ...
        magnets.append(
            Electromagnet(x, y, size=cell_size)
        )


# =========================================================
# --- PARTICLES (GAUSSIAN EM-CENTERED SPAWN) ---
# =========================================================

center_x = em_start_x + em_grid_width // 2
center_y = em_start_y + em_grid_height // 2

particles = []

spread = 20
margin = 3 * spread  # key idea: keep full gaussian inside bounds

min_x = maze_origin_x + 5
max_x = maze_origin_x + maze_width - 5
min_y = maze_origin_y + 5
max_y = maze_origin_y + maze_height - 5

edge = random.choice(["left", "right", "top", "bottom"])

if edge == "left":
    cluster_x = min_x + margin
    cluster_y = random.randint(min_y + margin, max_y - margin)

elif edge == "right":
    cluster_x = max_x - margin
    cluster_y = random.randint(min_y + margin, max_y - margin)

elif edge == "top":
    cluster_x = random.randint(min_x + margin, max_x - margin)
    cluster_y = min_y + margin

else:  # bottom
    cluster_x = random.randint(min_x + margin, max_x - margin)
    cluster_y = max_y - margin


for buh in range(15):

    x = int(random.gauss(cluster_x, spread))
    y = int(random.gauss(cluster_y, spread))
...
while running:
    ...
    # --- PHYSICS ---
    for p in particles:
        p.update_multi_physics(magnets, walls)
    ...
    txt = font.render("ACTIVE FIELD SYSTEM - MAZE TOPOLOGY", True, COLOR_TEXT)
